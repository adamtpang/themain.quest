...
const chartConfig = {
  score: { label: "Day score", color: "#7c6cf4" },
  quests: { label: "Quests", color: "#5fbf3a" },
} satisfies ChartConfig;

const categoryTone: Record<LifeQuest["category"], string> = {
  Stability: "bg-[#e7f4ff] text-[#216083]",
  Body: "bg-[#e6f6df] text-[#2c6f28]",
  Money: "bg-[#fff2c4] text-[#755b00]",
  Love: "bg-[#ffe5ef] text-[#9c315c]",
  Create: "bg-[#eee8ff] text-[#6147a8]",
...
function SidebarContent({ data, onClose }: { data: LifeCommandData; onClose?: () => void }) {
  return (
    <div className="sidebar flex h-full flex-col bg-[#18152b] text-white">
      <div className="flex h-20 items-center gap-3 px-6">
        <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#ffcf4a] font-pixel text-xs text-[#241b40]">MQ</div>
        <div>
      ...
      <div className="mx-4 mt-6 rounded-2xl border border-white/10 bg-white/[0.04] p-4">
        ...
        <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10">
          <div className="h-full rounded-full bg-[#ffcf4a] transition-all" style={{ width: `${data.levelProgress}%` }} />
        </div>
      ...
      <div className="mt-auto p-4">
        <div className="flex items-center gap-3 rounded-2xl border border-white/10 p-3">
          <Avatar className="h-9 w-9 border-0 bg-[#7cc9ff] text-[#241b40]">
            <AvatarFallback className="bg-[#7cc9ff] font-semibold text-[#241b40]">AP</AvatarFallback>
          </Avatar>
...
function MetricCard({
  ...
  return (
    <Card className="card metric border-0 shadow-sm ring-1 ring-black/[0.06]">
    ...
    <div className="min-h-screen lg:grid lg:grid-cols-[260px_1fr]">
      ...
      <div className="min-w-0 lg:col-start-2">
          ...
          <div className="flex h-16 items-center gap-3 px-4 sm:px-6 lg:px-8">
            ...
            <div className="ml-auto flex items-center gap-2">
              <div className="hidden items-center gap-2 rounded-full border bg-card px-3 py-1.5 text-xs text-muted-foreground sm:flex">
                <span className={cn("h-2 w-2 rounded-full", data.source === "starter" ? "bg-[#f6a623]" : "bg-[#5fbf3a]")} />
        ...
        <main className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
          {flash && <div className="fixed right-5 top-20 z-50 rounded-full bg-[#241b40] px-4 py-2 text-sm font-semibold text-white shadow-xl dark:bg-[#ffcf4a] dark:text-[#241b40]">{flash}</div>}
          ...
          <section id="today" className="scroll-mt-24">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5 text-[#5fbf3a]" /> Private player one</div>
            ...
            <Card className="relative mt-6 overflow-hidden border-0 bg-[#241b40] text-white shadow-xl shadow-[#241b40]/15">
              <div className="absolute right-0 top-0 h-64 w-64 rounded-full bg-[#7cc9ff]/10 blur-3xl" />
              <CardContent className="relative grid gap-6 p-6 lg:grid-cols-[1fr_auto] lg:items-center lg:p-8">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge className="border-0 bg-[#ffcf4a] text-[#241b40] hover:bg-[#ffcf4a]">Main quest</Badge>
                    {topOpenQuest && <Badge variant="outline" className="border-white/15 text-white/65">Tier {topOpenQuest.tier} · inaction {topOpenQuest.inactionCost}/10</Badge>}
                  </div>
                  ...
                  {topOpenQuest?.route && <div className="mt-4 inline-flex items-center gap-2 rounded-lg bg-white/[0.06] px-3 py-2 text-xs text-white/55"><Command className="h-3.5 w-3.5" /> Route to {topOpenQuest.route}</div>}
                </div>
                <div className="flex min-w-[220px] flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.05] p-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs uppercase tracking-[0.16em] text-white/45">Focus strike</span>
                    <Swords className="h-4 w-4 text-[#ffcf4a]" />
                  </div>
                  <p className="font-mono text-4xl font-semibold tabular-nums">{formatTimer(focusSeconds)}</p>
                  <div className="flex gap-2">
                    <Button className="btn-primary flex-1 bg-[#ffcf4a] text-[#241b40] hover:bg-[#ffe07c]" onClick={() => setFocusRunning((running) => !running)} disabled={!topOpenQuest}>
                      {focusRunning ? <><Clock3 /> Pause</> : <><Play /> Start</>}
                    </Button>
                    <Button variant="outline" size="icon" className="border-white/15 bg-transparent text-white hover:bg-white/10 hover:text-white" onClick={resetTimer}><TimerReset /></Button>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="section-cards mt-5 grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
              <MetricCard label="Today score" value={`${data.todayScore}/10`} detail={`${data.completedToday} real closes`} icon={Trophy} tone="bg-[#fff2c4] text-[#755b00]" />
              <MetricCard label="Current XP" value={data.xp.toLocaleString()} detail={`Level ${data.level} · ${data.xpToNext} to next`} icon={Zap} tone="bg-[#eee8ff] text-[#6147a8]" />
              <MetricCard label="High-tier open" value={String(data.quests.filter((quest) => !quest.completed).length)} detail={`${data.laterCount} safely parked`} icon={ListChecks} tone="bg-[#e7f4ff] text-[#216083]" />
              <MetricCard label="Life remaining" value={`${data.life.yearsLeft.toFixed(1)}y`} detail={`${data.life.daysLeft.toLocaleString()} days · 100-year map`} icon={Orbit} tone="bg-[#e6f6df] text-[#2c6f28]" />
            </div>
          </section>

          <section id="signals" className="mt-6 scroll-mt-24 grid gap-5 xl:grid-cols-[1.55fr_0.85fr]">
            <Card className="border-0 shadow-sm ring-1 ring-black/[0.06]">
          ...
          <section id="quests" className="mt-6 scroll-mt-24 grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
              ...
              <CardContent className="grid gap-2">
                {data.quests.map((quest, index) => (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    className={cn("group grid w-full grid-cols-[auto_1fr_auto] items-center gap-3 rounded-xl border p-3 text-left transition-all hover:bg-muted/55", selectedQuest?.id === quest.id && "border-primary/25 bg-secondary/35", quest.completed && "opacity-65")}
                  ...
                  >
                    ...
                    {quest.completed ? <Check className="h-4 w-4 text-[#3f9e2a]" /> : <ChevronRight className="h-4 w-4 text-muted-foreground transition-transform group-hover:translate-x-0.5" />}
                  </div>
                ))}
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-sm ring-1 ring-black/[0.06]">
              <CardHeader>
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#7c6cf4]"><Brain className="h-3.5 w-3.5" /> Clarity layer</div>
              ...
              <CardContent>
                ...
                {selectedQuest && !selectedQuest.completed && (
                  <Button className="mt-5 w-full bg-[#241b40] text-white hover:bg-[#322650] dark:bg-[#ffcf4a] dark:text-[#241b40]" onClick={() => toggleQuest(selectedQuest, true)}>
              ...
              <CardContent>
                <div className="grid auto-cols-[160px] grid-flow-col gap-3 overflow-x-auto pb-2 md:grid-flow-row md:grid-cols-3 md:overflow-visible xl:grid-cols-9">
                  {data.horizons.map((horizon, index) => (
                    <div key={horizon.label} className="relative rounded-xl border bg-background/60 p-3">
                      ...
                      <div className="h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-[#7c6cf4]" style={{ width: `${horizon.progress}%` }} /></div>
          ...
          <footer className="mt-8 flex flex-col gap-3 border-t py-6 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-[#3f9e2a]" /> Private life data is served only after authorization.</div>
