...
`lib/life-context.ts` builds the `# Personal context` brief (situation, problems, goals, priorities, constraints, patterns, guidance, open questions) from `questbook.md`, `deathguide.md`, the vault `🧙 CHARACTER.md`, and the live outbox, with only bold quest titles kept, the one move cut to its first clause, and money amounts replaced by `[amount]`. `npm run life:context` prints it; `npm run life:context -- --send` mails it to summon.guide as a repos.chat `notice` (subject `life-context`) via `repos.chat/repos.mjs send`. summon.guide's `/council` reads the newest notice, seats three guides, and attaches the brief to the chosen guide chat for that session only. `app/api/life-context/route.ts` is the owner-only GET. Re-send whenever the questbook or outbox changes. Verified 2026-09-05: 54 tests, clean `tsc`, real 638-word brief, notice delivered, `repos verify` green. See CLAUDE.md for the full note.
...
`C:\Users\adamp\Desktop\win\Unsorted\Folders\tags\flow.md`: activation lever -> ONE
